...
The PDF backend lives in `modules/report/ReportModule.cpp`.
...
**Files touched:** `ReportModule.cpp` and `app.js`.

#### B-1. Add the DB column

In `ReportModule.cpp` → `ensureSchema_()`:
...
|------|----------|---------|
| `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` | `ReportModule.cpp` `ensureSchema_()` | — |
| SELECT + serialise in `getSettingById_()` | `ReportModule.cpp` | — |
| Parse + UPDATE in `handleWrite()` | `ReportModule.cpp` | Update both WITH-NAME and WITHOUT-NAME UPDATE branches |
| Read in PDF route inline SELECT | `ReportModule.cpp` PDF lambda | — |
