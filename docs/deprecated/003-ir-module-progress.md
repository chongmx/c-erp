...
## Summary
Implemented the reference ERP IR (Information Repository) layer. The frontend is now database-driven: menus load from `ir.ui.menu`, actions load from `ir.actions.act_window`, and views (arch + fields) are served from the C++ ViewFactory. Phase 5 is complete.
...
Handles:
- `load_menus(debug)` — returns flat dict keyed by id (string) + `"root"` sentinel, the reference ERP webclient format:
  ```json
  {
