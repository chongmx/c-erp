...
Reference: `zzref/odoo/` (the reference ERP.0 source)
...
- reference-ERP-style navigation: home screen app tiles → horizontal top nav with dropdowns
...
The C++ `BaseModel<T>` covers the core ORM operations the reference ERP uses most:
...
the reference ERP's OWL webclient cannot render anything after login without the **IR layer**.
...
with known model names. So IR stubs are needed only when targeting the real the reference ERP client.
...
Currently `res_company` has no `partner_id`. the reference ERP's company is always backed by a partner.
```sql
...
the reference ERP's webclient expects this shape:
...
These are **not needed** for the custom frontend. Port them when targeting full the reference ERP client compatibility.
