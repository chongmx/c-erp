...
**Assessment:** Confirmed. The dispatcher checks only `session.isAuthenticated()` — there is no model-level access control (no equivalent of the reference ERP's `ir.model.access` or `ir.rule`).
