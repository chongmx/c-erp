...
regardless of ownership, company, or sales team.  In the reference ERP this layer is provided by
...
Implemented a full the reference ERP-compatible record-rule engine.  The implementation follows the
same semantics as the reference ERP's Python `ir.rule`:

| the reference ERP concept | This implementation |
|---|---|
...
- **OR** of two domains: the reference ERP prefix notation `["|", ...d1_items, ...d2_items]` with `"&"` grouping for multi-leaf sub-domains
...
- Admin users (`isAdmin=true`, group id=3) bypass all rules unconditionally.
  This prevents admins from locking themselves out and matches the reference ERP superuser semantics.
