...
`errorResponse_` gained an optional `name` parameter (5th arg, default `"cerp.exceptions.UserError"`) so the frontend can distinguish error types.
...
- Catches `err.type === 'cerp.exceptions.ConcurrencyConflict'` → sets `state.conflictError` (shows a yellow banner with a **Reload** button).
