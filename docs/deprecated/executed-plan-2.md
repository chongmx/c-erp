...
### Why first
`product.uom_id` is NOT NULL in the reference ERP. Both sale lines and purchase lines reference `product_uom_id`. Without UOM the product model can't be saved correctly.

### Simplifications vs the reference ERP
...
**Keep: one table per "model"** — skip the `product.template` + `product.product` split. Use a single `ProductProduct` class that IS both template and variant (like the reference ERP in simple mode). This matches how single-variant products work.
...
`ProductProduct::name` field: also expose as `product_tmpl_id` self-reference = id for the reference ERP compat.
