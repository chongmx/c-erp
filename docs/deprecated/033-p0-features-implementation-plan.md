...
Standard the reference ERP onchange: client sends `{ method: 'onchange', args: [currentValues, changedFieldName, fieldsSpec] }`. Server returns `{ "value": { field: newVal, ... }, "warning": null }`. Frontend merges `value` into form state.
