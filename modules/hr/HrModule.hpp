#pragma once
// =============================================================
// modules/hr/HrModule.hpp
//
// Phase 12 — Human Resources module
//
// Models:  resource.calendar  (table: resource_calendar)
//          hr.department      (table: hr_department)
//          hr.job             (table: hr_job)
//          hr.employee        (table: hr_employee)
//
// Seeds: 1 default working schedule (Standard 40h/week)
// IR: ir_act_window id=13 (Employees), id=14 (Departments)
//     ir_ui_menu id=80 (HR app), id=81 (Employees), id=82 (Departments)
//     id=83 (Configuration section), id=84 (Jobs), id=85 (Work Schedules)
// =============================================================
#include "IModule.hpp"
...
public:
    ...
    explicit HrJob(std::shared_ptr<DbConnection> db)
        : BaseModel(std::move(db)) {}
...
public:
    ...
    explicit HrEmployee(std::shared_ptr<DbConnection> db)
        : BaseModel(std::move(db)) {}

    std::string name;
    int         jobId              = 0;
    int         departmentId       = 0;
    int         parentId           = 0;   // manager (hr.employee)
    int         coachId            = 0;   // coach  (hr.employee)
    ...
    int         companyId          = 1;
    int         userId             = 0;
    int         addressId          = 0;   // work address (res.partner)
    std::string gender;                   // male / female / other
    std::string marital;                  // single / married / other
    std::string birthday;
    std::string identificationId;         // national ID
...
public:
    ...
    std::string arch() const override {
        return "<list string=\"Working Schedules\">"
               "<field name=\"name\"/>"
    ...
    nlohmann::json fields() const override {
        return {
            {"name",          {{"type","char"},    {"string","Working Schedule"}}},
...
public:
    ...
    std::string arch() const override {
        return "<form string=\"Working Schedule\">"
               "<field name=\"name\"/>"
               "<field name=\"hours_per_day\"/>"
               "<field name=\"company_id\"/>"
               "<field name=\"active\"/>"
...
public:
    ...
    std::string viewType()  const override { return "list"; }
    std::string arch() const override {
        return "<list string=\"Departments\">"
               "<field name=\"name\"/>"
    ...
    nlohmann::json fields() const override {
        return {
            {"name",       {{"type","char"},    {"string","Department"}}},
...
public:
    ...
    std::string viewType()  const override { return "list"; }
    std::string arch() const override {
        return "<list string=\"Job Positions\">"
               "<field name=\"name\"/>"
    ...
    nlohmann::json fields() const override {
        return {
            {"name",          {{"type","char"},    {"string","Job Position"}}},
...
public:
    ...
    std::string viewType()  const override { return "form"; }
    std::string arch() const override {
        return "<form string=\"Job Position\">"
               "<field name=\"name\"/>"
               "<field name=\"department_id\"/>"
               "<field name=\"company_id\"/>"
...
public:
    ...
    explicit HrModule(ModelFactory&     modelFactory,
    ...
    std::string              moduleName()   const override { return "hr"; }
    std::string              version()      const override { return "19.0.1.0.0"; }
    std::vector<std::string> dependencies() const override { return {"base", "auth"}; }
...
private:
    ...
    void ensureSchema_() {
        ...
        // resource_calendar — must come first (hr_employee refs it)
        txn.exec(R"(
        ...
        // hr_department — self-referential (manager_id added after hr_employee)
        txn.exec(R"(
        ...
        // Add manager_id FK to hr_department now that hr_employee exists
        txn.exec(R"(
    ...
    void seedDefaults_() {
        ...
        // Standard 40h/week working schedule
        txn.exec(R"(
    ...
    void seedMenus_() {
        ...
        // ir_act_window id=14: Departments
        txn.exec(R"(
            INSERT INTO ir_act_window (id, name, res_model, view_mode, context, target)
        ...
        // ir_act_window id=15: Job Positions
        txn.exec(R"(
            INSERT INTO ir_act_window (id, name, res_model, view_mode, context, target)
        ...
        // ir_act_window id=16: Working Schedules
        txn.exec(R"(
            INSERT INTO ir_act_window (id, name, res_model, view_mode, context, target)
        ...
        // Level 0 — HR app tile (id=80, parent=NULL)
        txn.exec(R"(
        ...
        // Level 1 — Employees direct leaf (id=81, parent=80, action=13)
        txn.exec(R"(
        ...
        // Level 1 — Departments direct leaf (id=82, parent=80, action=14)
        txn.exec(R"(
            INSERT INTO ir_ui_menu (id, name, parent_id, sequence, action_id)
        ...
        // Level 1 — Configuration section (id=83, parent=80, no action)
        txn.exec(R"(
            INSERT INTO ir_ui_menu (id, name, parent_id, sequence, action_id)
        ...
        // Level 2 — Job Positions (id=84, parent=83, action=15)
        txn.exec(R"(
            INSERT INTO ir_ui_menu (id, name, parent_id, sequence, action_id)
        ...
        // Level 2 — Working Schedules (id=85, parent=83, action=16)
        txn.exec(R"(
            INSERT INTO ir_ui_menu (id, name, parent_id, sequence, action_id)
