...
public:
    ...
    void registerFields() {
        ...
        fieldRegistry_.add({.name="invoice_status", .type=FieldType::Selection, .string="Invoice Status", .readonly=true, .compute=true, .depends="order_line"});
        ...
        fieldRegistry_.markScaled({"amount_untaxed", "amount_tax", "amount_total"});  // P2: 920
    }
...
public:
    ...
    void registerFields() {
        ...
        fieldRegistry_.add({.name="qty_invoiced",   .type=FieldType::Float,    .string="Invoiced Qty",  .readonly=true, .compute=true, .depends="order_id.invoice_ids"});
        fieldRegistry_.add({.name="qty_delivered",  .type=FieldType::Float,    .string="Delivered Qty", .readonly=true, .compute=true, .depends="order_id.picking_ids"});
        fieldRegistry_.add({"company_id",      FieldType::Many2one,  "Company",         false, false, true, false, "res.company"});
        fieldRegistry_.add({"currency_id",     FieldType::Many2one,  "Currency",        false, false, true, false, "res.currency"});
        // P2: BIGINT micro-units (migration 921)
        ...
        fieldRegistry_.markScaled({"price_unit", "product_uom_qty", "discount",
...
protected:
    ...
    // Helper: extract an integer id from int, [id,"Name"], or null/false
    static int saleOnchangeId(const nlohmann::json& v) {
        if (v.is_number_integer()) return v.get<int>();
...
private:
    ...
    nlohmann::json handleActionConfirm(const CallKwArgs& call) {
        ...
        for (int id : ids) {
            ...
            // P4: ir.sequence, allocated inside THIS transaction. The old
            // nextval() burned a number on rollback because PG sequences are
            // deliberately non-transactional; sharing the caller's txn means
            // the number returns if the confirm fails.
            std::ostringstream ss;
            ss << core::IrSequence::instance().nextByCode(txn, "sale.order");
            ...
            if (!ptRow.empty()) {
                ...
                const std::string pickName =                       // P4
                    core::IrSequence::instance().nextByCode(txn, "stock.picking.out");
...
public:
    ...
    // onchange — product_id: fill name, price_unit, product_uom_id
    nlohmann::json handleOnchange(const CallKwArgs& call) override {
        ...
        if (fld == "product_id") {
            const int productId = values.contains("product_id")
                                  ? saleOnchangeId(values["product_id"]) : 0;
...
private:
    // P3: recompute via TaxEngine.
    //
    // What was here handled `percent && !price_include` only, so a
    // price-INCLUDED tax contributed nothing: the whole gross landed in the
    // subtotal and the order reported zero tax. It also worked in double with
    // its own rounding, which P2 replaced everywhere else.
    void recomputeAmounts_(nlohmann::json& vals) {
        auto conn = db_->acquire();
        pqxx::work txn{conn.get()};
        core::applyLineTaxes(txn, vals, "product_uom_qty");
        txn.commit();
    }
...
SaleModule::SaleModule(ModelFactory&     modelFactory,
...
std::string SaleModule::moduleName() const { return "sale"; }
std::string SaleModule::version()    const { return "19.0.1.0.0"; }
std::vector<std::string> SaleModule::dependencies() const { return {"product", "account"}; }

void SaleModule::registerModels() {
...
void SaleModule::registerServices() {}
void SaleModule::registerRoutes()   {}

void SaleModule::registerViews() {
...
void SaleModule::registerViewModels() {
    auto db = services_.db();
...
void SaleModule::initialize() {
    ensureSchema_();
    seedMenus_();
}

void SaleModule::ensureSchema_() {
